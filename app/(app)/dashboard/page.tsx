...
type DashboardData = {
  ...
  revenueByDay: { date: string; label: string; revenueCents: number }[];
...
export default function DashboardPage() {
  ...
  return <div className="space-y-4 p-3 lg:p-5 animate-fade-in">
    ...
    {data && !error && <>
      ...
      {data.teamPerformance.length > 0 && <Card className="p-4"><h2 className="font-heading text-base font-semibold">Performance por vendedor</h2><p className="mb-3 text-xs text-muted-foreground">Leads recebidos, atendimento, fechamentos, receita e ticket médio por Atendente/Vendedor.</p><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="text-left text-xs uppercase text-muted-foreground"><tr><th className="py-2">Vendedor</th><th>Leads recebidos</th><th>Em atendimento</th><th>Fechamentos</th><th>Receita</th><th>Conversão</th><th>Ticket médio</th><th>Tempo médio até fechamento</th></tr></thead><tbody>{data.teamPerformance.map((seller) => <tr key={seller.userId} className="border-t"><td className="py-2.5 font-medium">{seller.name}</td><td>{seller.leadsReceived}</td><td>{seller.inProgress}</td><td>{seller.won}</td><td>{moneyFromCents(seller.revenueCents)}</td><td>{seller.conversionRate}%</td><td>{moneyFromCents(seller.averageTicketCents)}</td><td>{seller.averageCloseTimeHours == null ? '—' : `${seller.averageCloseTimeHours}h`}</td></tr>)}</tbody></table></div></Card>}
