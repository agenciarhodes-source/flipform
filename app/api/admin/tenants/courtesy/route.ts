...
const slugify = (v: string) =>
  v.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
const validEmail = (email: string) => /.+@.+\..+/.test(email);

async function planBySlug(slug?: string) {
  return (
    (slug ? await prisma.plan.findFirst({ where: { slug, isActive: true } }) : null) ||
...
export async function POST(req: Request) {
  try {
    ...
    if (!session || session.globalRole !== 'platform_admin') {
      return adminError('Não autorizado', 403, { code: 'UNAUTHORIZED' });
    }
    ...
    const created = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      const tenant = await tx.tenant.create({
        data: {
          ...
          status: 'active',
          planId: plan.id,
          internalNotes: `Acesso de cortesia (${ownerName || ownerEmail})`,
        },
      });
      ...
      const user = await tx.user.upsert({
        ...
        create: {
          ...
          passwordHash: 'RESET_REQUIRED_NO_PASSWORD',
        },
