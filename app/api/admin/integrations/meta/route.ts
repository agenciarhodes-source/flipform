...
const schema = z.object({
  ...
  instagramAppId: z.string().trim().max(128).optional(),
  ...
  whatsappAdminSystemUserAccessToken: z.string().trim().min(1).max(8192).optional(),
  whatsappSystemUserAccessToken: z.string().trim().min(1).max(8192).optional(),
