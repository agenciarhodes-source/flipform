...
export const DELETE = withPermission('SETTINGS_EDIT', async (_req, session, ctx: { params: { id: string } }) => {
  ...
  await logAudit({ tenantId: session.tenantId, userId: session.userId, entityType: 'custom_form_domain', entityId: domain.id, action: 'custom_form_domain.deleted', metadata: { domain: domain.domain } });
