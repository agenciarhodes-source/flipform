...
type ColumnInfo = { column_name: string; data_type: string; udt_name: string; is_nullable: string; column_default: string | null };
...
export async function runAdminSchemaReadinessChecks(): Promise<AdminSchemaCheck[]> {
      ...
      add(checks, {
        ...
        suggestion: table === 'forms' && column === 'disqualification_settings'
          ...
          : table === 'leads' && column === 'sale_value_cents'
            ...
            : table === 'leads'
              ? (column === 'state' || column === 'city' ? 'leads.state/city ausentes. Rode migration ou repair-production-schema.' : `leads.${column} ausente. Rode migration ou repair schema.`)
              : table === 'lead_purchases' ? 'lead_purchases ausente ou incompleta. Rode migration ou repair-production-schema.' : table.startsWith('lead_assignment_rotation') ? 'lead_assignment_rotations ausente ou incompleta. Rode migration ou repair-production-schema.' : `ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${column} ...;`,
        runtimeEssential: RUNTIME_REQUIRED_TABLES.has(table),
      });
    }
  }
  ...
  const enteredAt = columnMaps.get('leads')?.get('entered_at');
  const hasLeadsTenantEnteredAt = typedIndexes.some((idx) => idx.indexname === 'leads_tenant_id_entered_at_idx' && idx.tablename === 'leads' && indexHasColumn(idx.indexdef, 'tenant_id') && indexHasColumn(idx.indexdef, 'entered_at'));
  ...
  const hasTrackingLogsTenantTriggerRuleEvent = typedIndexes.some((idx) => idx.indexname === 'tracking_event_logs_tenant_trigger_rule_event_idx' && idx.tablename === 'tracking_event_logs' && indexHasColumn(idx.indexdef, 'tenant_id') && indexHasColumn(idx.indexdef, 'trigger_rule_id') && indexHasColumn(idx.indexdef, 'event_name'));
  const hasTrackingLogsTenantConversation = typedIndexes.some((idx) => idx.indexname === 'tracking_event_logs_tenant_conversation_idx' && idx.tablename === 'tracking_event_logs' && indexHasColumn(idx.indexdef, 'tenant_id') && indexHasColumn(idx.indexdef, 'conversation_id'));
  const hasWhatsAppTriggersTenant = typedIndexes.some((idx) => idx.indexname === 'whatsapp_event_triggers_tenant_id_idx' && idx.tablename === 'whatsapp_event_triggers' && indexHasColumn(idx.indexdef, 'tenant_id'));
  const hasWhatsAppTriggersTenantEnabledOrder = typedIndexes.some((idx) => idx.indexname === 'whatsapp_event_triggers_tenant_enabled_order_idx' && idx.tablename === 'whatsapp_event_triggers' && indexHasColumn(idx.indexdef, 'tenant_id') && indexHasColumn(idx.indexdef, 'enabled') && indexHasColumn(idx.indexdef, 'order_index'));
  ...
  add(checks, { label: 'column.leads.entered_at.not_null', ok: enteredAt?.is_nullable === 'NO', suggestion: 'ALTER TABLE leads ALTER COLUMN entered_at SET NOT NULL;', runtimeEssential: false });
  add(checks, { label: 'column.leads.entered_at.default', ok: /current_timestamp|now\(\)/i.test(enteredAt?.column_default || ''), suggestion: 'ALTER TABLE leads ALTER COLUMN entered_at SET DEFAULT CURRENT_TIMESTAMP;', runtimeEssential: false });
  add(checks, { label: 'index.leads.tenant_id_entered_at', ok: hasLeadsTenantEnteredAt, suggestion: 'CREATE INDEX IF NOT EXISTS leads_tenant_id_entered_at_idx ON leads(tenant_id, entered_at);', runtimeEssential: false });
  ...
  add(checks, { label: 'index.tracking_event_logs.tenant_trigger_rule_event', ok: hasTrackingLogsTenantTriggerRuleEvent, suggestion: 'CREATE INDEX IF NOT EXISTS tracking_event_logs_tenant_trigger_rule_event_idx ON tracking_event_logs(tenant_id, trigger_rule_id, event_name);', runtimeEssential: false });
  add(checks, { label: 'index.tracking_event_logs.tenant_conversation', ok: hasTrackingLogsTenantConversation, suggestion: 'CREATE INDEX IF NOT EXISTS tracking_event_logs_tenant_conversation_idx ON tracking_event_logs(tenant_id, conversation_id);', runtimeEssential: false });
  add(checks, { label: 'index.whatsapp_event_triggers.tenant_id', ok: hasWhatsAppTriggersTenant, suggestion: 'CREATE INDEX IF NOT EXISTS whatsapp_event_triggers_tenant_id_idx ON whatsapp_event_triggers(tenant_id);', runtimeEssential: false });
  add(checks, { label: 'index.whatsapp_event_triggers.tenant_enabled_order', ok: hasWhatsAppTriggersTenantEnabledOrder, suggestion: 'CREATE INDEX IF NOT EXISTS whatsapp_event_triggers_tenant_enabled_order_idx ON whatsapp_event_triggers(tenant_id, enabled, order_index);', runtimeEssential: false });
