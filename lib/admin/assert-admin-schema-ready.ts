...
const REQUIRED_TABLES = ['users', 'tenants', 'tenant_users', 'allowed_users', 'plans', 'subscriptions', 'audit_logs', 'payments', 'tenant_integration_settings', 'kanban_stage_tracking_events', 'tracking_event_logs', 'whatsapp_event_triggers', 'custom_form_domains', 'custom_form_domain_routes'];
...
const REQUIRED_COLUMNS: Record<string, string[]> = {
  ...
  custom_form_domains: ['id', 'tenant_id', 'domain', 'status', 'verification_status', 'ssl_status', 'vercel_project_id', 'vercel_verified', 'verification_type', 'verification_domain', 'verification_value', 'verification_reason', 'dns_target', 'default_form_id', 'last_checked_at', 'verified_at', 'created_at', 'updated_at'],
  custom_form_domain_routes: ['id', 'tenant_id', 'domain_id', 'form_id', 'path', 'is_default', 'created_at', 'updated_at'],
};
...
export async function runAdminSchemaReadinessChecks(): Promise<AdminSchemaCheck[]> {
  ...
  const indexes = await prisma.$queryRaw<IndexInfo[]>`
    ...
    where schemaname = 'public'
      and tablename in ('users', 'tenants', 'tenant_users', 'allowed_users', 'plans', 'subscriptions', 'payments', 'tenant_integration_settings', 'kanban_stage_tracking_events', 'tracking_event_logs', 'whatsapp_event_triggers', 'custom_form_domains', 'custom_form_domain_routes')
  `;
