...
type StatePayload = { nonce: string; tenantId: string; userId: string; expiresAt: number };
...
export function createMetaOAuthState(tenantId: string, userId: string, now = Date.now()) {
  const payload: StatePayload = { nonce: crypto.randomBytes(32).toString('base64url'), tenantId, userId, expiresAt: now + META_OAUTH_STATE_TTL_SECONDS * 1000 };
  const encoded = Buffer.from(JSON.stringify(payload)).toString('base64url');
  return { state: payload.nonce, cookie: `${encoded}.${sign(encoded)}` };
}

export function verifyMetaOAuthState(cookie: string | undefined, state: string | null, tenantId: string, userId: string, now = Date.now()) {
  ...
  try {
    ...
    return payload.nonce === state && payload.tenantId === tenantId && payload.userId === userId && payload.expiresAt > now;
  } catch { return false; }
}
