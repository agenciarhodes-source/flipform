...
const steps: Step[] = [
  ...
  { label: 'leads.entered_at', sql: `ALTER TABLE public.leads ADD COLUMN IF NOT EXISTS entered_at TIMESTAMP(3)` },
  { label: 'leads.entered_at.backfill', sql: `UPDATE public.leads SET entered_at = created_at WHERE entered_at IS NULL` },
  { label: 'leads.entered_at.default', sql: `ALTER TABLE public.leads ALTER COLUMN entered_at SET DEFAULT CURRENT_TIMESTAMP` },
  { label: 'leads.entered_at.not_null', sql: `ALTER TABLE public.leads ALTER COLUMN entered_at SET NOT NULL` },
  { label: 'leads.tenant_entered_at.index', sql: `CREATE INDEX IF NOT EXISTS leads_tenant_id_entered_at_idx ON public.leads(tenant_id, entered_at)` },
...
steps.push(
  ...
  { label: 'lead_attributions.table', sql: `CREATE TABLE IF NOT EXISTS public.lead_attributions (id TEXT PRIMARY KEY DEFAULT md5(random()::text || clock_timestamp()::text), tenant_id TEXT NOT NULL, lead_id TEXT NOT NULL, utm_source TEXT, utm_medium TEXT, utm_campaign TEXT, utm_content TEXT, utm_term TEXT, fbclid TEXT, fbc TEXT, fbp TEXT, gclid TEXT, landing_page TEXT, referrer TEXT, client_ip TEXT, client_user_agent TEXT, captured_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP)` },
  { label: 'lead_attributions.columns', sql: `ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS id TEXT DEFAULT md5(random()::text || clock_timestamp()::text); ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS tenant_id TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS lead_id TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS utm_source TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS utm_medium TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS utm_campaign TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS utm_content TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS utm_term TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS fbclid TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS fbc TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS fbp TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS gclid TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS landing_page TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS referrer TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS client_ip TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS client_user_agent TEXT; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS captured_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP; ALTER TABLE public.lead_attributions ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP` },
  { label: 'lead_attributions.indexes', sql: `CREATE UNIQUE INDEX IF NOT EXISTS lead_attributions_lead_id_key ON public.lead_attributions(lead_id); CREATE INDEX IF NOT EXISTS lead_attributions_tenant_id_idx ON public.lead_attributions(tenant_id); CREATE INDEX IF NOT EXISTS lead_attributions_tenant_id_utm_source_idx ON public.lead_attributions(tenant_id, utm_source); CREATE INDEX IF NOT EXISTS lead_attributions_tenant_id_utm_campaign_idx ON public.lead_attributions(tenant_id, utm_campaign); CREATE INDEX IF NOT EXISTS lead_attributions_tenant_id_captured_at_idx ON public.lead_attributions(tenant_id, captured_at)` },
  { label: 'lead_attributions.fkeys', sql: `DO $$ BEGIN IF to_regclass('public.tenants') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'lead_attributions_tenant_id_fkey') THEN ALTER TABLE public.lead_attributions ADD CONSTRAINT lead_attributions_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES public.tenants(id) ON DELETE CASCADE; END IF; IF to_regclass('public.leads') IS NOT NULL AND NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'lead_attributions_lead_id_fkey') THEN ALTER TABLE public.lead_attributions ADD CONSTRAINT lead_attributions_lead_id_fkey FOREIGN KEY (lead_id) REFERENCES public.leads(id) ON DELETE CASCADE; END IF; END $$;` }
