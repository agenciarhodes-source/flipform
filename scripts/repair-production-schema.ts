...
const defaultPlans: Step[] = [
  ...
  { label: 'custom_form_domains.default_form_id', sql: `ALTER TABLE custom_form_domains ADD COLUMN IF NOT EXISTS default_form_id TEXT` },
  ...
  {
    label: 'custom_form_domain_routes.create',
    sql: `CREATE TABLE IF NOT EXISTS custom_form_domain_routes (
      id TEXT PRIMARY KEY DEFAULT md5(random()::text || clock_timestamp()::text),
      tenant_id TEXT NOT NULL,
  ...
  { label: 'custom_form_domain_routes.id', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS id TEXT DEFAULT md5(random()::text || clock_timestamp()::text)` },
  { label: 'custom_form_domain_routes.tenant_id', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS tenant_id TEXT` },
  { label: 'custom_form_domain_routes.domain_id', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS domain_id TEXT` },
  { label: 'custom_form_domain_routes.form_id', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS form_id TEXT` },
  { label: 'custom_form_domain_routes.path', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS path TEXT NOT NULL DEFAULT '/'` },
  { label: 'custom_form_domain_routes.is_default', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS is_default BOOLEAN NOT NULL DEFAULT false` },
  { label: 'custom_form_domain_routes.created_at', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT now()` },
  { label: 'custom_form_domain_routes.updated_at', sql: `ALTER TABLE custom_form_domain_routes ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITHOUT TIME ZONE NOT NULL DEFAULT now()` },
  { label: 'custom_form_domain_routes.tenant.index', sql: `CREATE INDEX IF NOT EXISTS custom_form_domain_routes_tenant_id_idx ON custom_form_domain_routes(tenant_id)` },
  { label: 'custom_form_domain_routes.domain.index', sql: `CREATE INDEX IF NOT EXISTS custom_form_domain_routes_domain_id_idx ON custom_form_domain_routes(domain_id)` },
  { label: 'custom_form_domain_routes.form.index', sql: `CREATE INDEX IF NOT EXISTS custom_form_domain_routes_form_id_idx ON custom_form_domain_routes(form_id)` },
  { label: 'custom_form_domain_routes.domain_path.unique', sql: `CREATE UNIQUE INDEX IF NOT EXISTS custom_form_domain_routes_domain_id_path_key ON custom_form_domain_routes(domain_id, path)` },
