-- Repair admin manual access schema without deleting production data.
-- This migration is intentionally idempotent and non-destructive: it only adds
-- missing compatibility columns, relaxes nullable invited_by, removes the legacy
-- global allowed_users.email uniqueness, creates tenant-scoped uniqueness, and
-- inserts missing default plans.
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_type WHERE typname = 'TenantStatus')
     AND NOT EXISTS (
       SELECT 1
       FROM pg_enum e
       JOIN pg_type t ON e.enumtypid = t.oid
    ...
    ALTER TYPE "TenantStatus" ADD VALUE 'active';
  END IF;
END $$;
...
-- Tenants expected by internal/manual courtesy access.
...
-- Allowed users must be tenant-scoped, not globally unique by e-mail.
...
-- Subscriptions required for courtesy/manual billing state.
...
-- Plans required by the admin access selector and manual access creation.
ALTER TABLE plans ADD COLUMN IF NOT EXISTS name TEXT NOT NULL DEFAULT 'Plano';
ALTER TABLE plans ADD COLUMN IF NOT EXISTS slug TEXT;
ALTER TABLE plans ADD COLUMN IF NOT EXISTS description TEXT;
ALTER TABLE plans ADD COLUMN IF NOT EXISTS price NUMERIC(10, 2) NOT NULL DEFAULT 0;
...
INSERT INTO plans (id, name, slug, description, price, billing_cycle, max_users, max_forms, max_leads_per_month, max_pipelines, can_use_reports, can_export_csv, can_use_custom_branding, can_use_meta_pixel, can_use_webhooks, can_use_tasks, is_active, created_at, updated_at)
